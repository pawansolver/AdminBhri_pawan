...
export default function EventsPage() {
  ...
  const resetForm = () => {
    setTitle(""); setDescription(""); setEventDate(""); setEventTime(""); setVenue(""); setIsActive(true);
  ...
  const handleSave = async () => {
    ...
    if (data.success) {
      setShowForm(false); setEditId(null); fetchEvents(); resetForm();
    } else {
  ...
  const handleEdit = (item: EventItem) => {
    setEditId(item.id);
    setTitle(item.title); setDescription(item.description); setEventDate(item.eventDate);
    setEventTime(item.eventTime || ""); setVenue(item.venue || ""); setIsActive(item.isActive);
    setThumbnailFile(null); setPdfFile(null);
    setShowForm(true);
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    await fetch(`${API_BASE}/events/${id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token()}` } });
    fetchEvents();
  };
  ...
  const totalPages = Math.ceil(events.length / rowsPerPage) || 1;
  const paginated = events.slice((currentPage - 1) * rowsPerPage, currentPage * rowsPerPage);

  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <h1 className="text-2xl font-bold text-gray-800 !text-gray-800">Events</h1>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-xl p-6 w-full max-w-2xl shadow-2xl max-h-[90vh] overflow-y-auto">
            <h3 className="text-lg font-bold mb-4 !text-gray-800 border-b pb-2">{editId ? "Edit Event" : "Add Event"}</h3>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="col-span-full">
                <label className="text-xs font-bold text-gray-600 uppercase">Title *</label>
                <input value={title} onChange={e => setTitle(e.target.value)} className="w-full px-3 py-2 border rounded-lg text-sm mt-1" placeholder="Event title" />
              </div>
              <div className="col-span-full">
                <label className="text-xs font-bold text-gray-600 uppercase">Description *</label>
                <textarea rows={3} value={description} onChange={e => setDescription(e.target.value)} className="w-full px-3 py-2 border rounded-lg text-sm mt-1" placeholder="Event description..."></textarea>
              </div>
              <div>
                <label className="text-xs font-bold text-gray-600 uppercase">Event Date *</label>
                <input type="date" value={eventDate} onChange={e => setEventDate(e.target.value)} className="w-full px-3 py-2 border rounded-lg text-sm mt-1" />
              </div>
              <div>
                <label className="text-xs font-bold text-gray-600 uppercase">Event Time</label>
                <input type="time" value={eventTime} onChange={e => setEventTime(e.target.value)} className="w-full px-3 py-2 border rounded-lg text-sm mt-1" />
              </div>
              <div className="col-span-full">
                <label className="text-xs font-bold text-gray-600 uppercase">Venue</label>
                <input value={venue} onChange={e => setVenue(e.target.value)} className="w-full px-3 py-2 border rounded-lg text-sm mt-1" placeholder="e.g. OPD Ground, Auditorium" />
              </div>
              <div>
      ...
      {loading ? <div className="text-center py-12 text-gray-400">Loading...</div> : (
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div key={item.id} className={`bg-white rounded-xl border p-0 shadow-sm flex flex-col overflow-hidden hover:shadow-md transition ${!item.isActive ? 'opacity-70 grayscale-[50%]' : 'border-gray-200'}`}>
                ...
                <div className="p-4 flex-1 flex flex-col">
                  <div className="flex justify-between items-start mb-2">
                    <p className="text-xs text-gray-500 font-semibold">{item.eventDate} {item.eventTime ? `• ${item.eventTime}` : ""}</p>
                  ...
                  {item.venue && <p className="text-xs text-gray-400 mt-1">{item.venue}</p>}
            ...
            {events.length === 0 && (
              <div className="col-span-full text-center py-12 text-gray-500 bg-white rounded-xl border border-dashed">No events. Click &quot;Add Event&quot; to create one.</div>
            )}
          </div>
